...
public class FileService {
	...
	@Async
	public Integer findKeyCount() {
		try {
			ListObjectsV2Result allObjects = amazonS3.listObjectsV2(s3BucketName);	
			List<S3ObjectSummary> summ = allObjects.getObjectSummaries();
			summ.forEach(sum-> LOG.info("key {} size {} eTag {}",sum.getKey(), sum.getSize(), sum.getETag()));
			return allObjects.getKeyCount();
		} catch(AmazonServiceException e) {
			LOG.error("Error {} occurred while uploading file", e.getLocalizedMessage());
		}
		return 0;
